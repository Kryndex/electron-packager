...
util.packagerTest('all test', (t) => {
  ...
  const NON_WIN32_ADMIN_EXPECTED_PACKAGES = 5
  const opts = createMultiTargetOptions({all: true})
  const message = 'Packages should be generated for all possible platforms'
  let expectedPackageCount = EXPECTED_PACKAGES

  if (process.platform === 'win32') {
    return isAdmin().then(admin => {
      if (!admin) expectedPackageCount = NON_WIN32_ADMIN_EXPECTED_PACKAGES
      return createMultiTargetPromise(t, opts, expectedPackageCount, message)
    })
  } else {
    return createMultiTargetPromise(t, opts, expectedPackageCount, message)
  }
